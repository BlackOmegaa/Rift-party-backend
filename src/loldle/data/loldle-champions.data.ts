/**
 * Liste complete des champions (memes noms que frontend/src/app/shared/lol-assets.ts,
 * gardee en synchro manuellement puisque backend et frontend n'ont pas de package partage).
 */
...
/** Ne garde que les lettres, en majuscules : "Kai'Sa" -> "KAISA", "Xin Zhao" -> "XINZHAO". */
...
export const LOLDLE_CHAMPIONS: LoldleChampion[] = CHAMPION_NAMES.map((name) => ({
  name,
  normalized: normalizeChampionName(name),
}));

/** Bassin des secrets possibles : on evite les noms trop courts (2-3 lettres, trop triviaux). */
export const LOLDLE_SECRET_POOL: LoldleChampion[] = LOLDLE_CHAMPIONS.filter(
  (c) => c.normalized.length >= 4,
);
